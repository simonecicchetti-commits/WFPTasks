...
RBP_IDB/
├── modules/
│   ├── fetcher/          # Data ingestion from external APIs
│   │   └── src/domain/pipes/
│   │       ├── hazard_pipe.py      # ADAM API (earthquakes, floods, cyclones)
│   │       ├── conflict_pipe.py    # ACLED conflict data
│   │       ├── economic_pipe.py    # Food inflation, currency
│   │       └── ...
│   └── trigger/          # Alert generation system
...
- Line 97: `db.flood_table` → `db.earthquake_table`
- Line 98: `"floods were NOT updated"` → `"earthquakes were NOT updated"`
- Line 132: REMOVED (was incorrectly appending failure in success block)
- Line 135: `db.flood_table` → `db.cyclone_table`
- Line 136: `"floods were NOT updated"` → `"cyclones were NOT updated"`
...
WFPTasks/
├── dashboard/
│   ├── app.py              # Streamlit dashboard (560 lines)
│   └── requirements.txt    # Dependencies
├── scripts/
│   └── db_explorer.py      # Database health monitoring (760 lines)
├── docs/
│   └── SESSION_2026_01_14_01.md  # This file
├── .gitignore
└── README.md
```
...
- Stored in: GitHub Settings → Developer settings → Personal access tokens
...
| File | Path |
|------|------|
| db_explorer.py | `/Users/simonecicchetti/WFP-Projects/WFPTasks/scripts/db_explorer.py` |
